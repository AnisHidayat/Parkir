...
if ($page=='tambahparkir' AND $act=='parkiran'){
					...
					$cek_parkir=mysqli_query($con, "SELECT id_parkir from tempat_parkir WHERE blokir='N' AND tipe_parkir='Motor' ORDER BY RAND() LIMIT 1");
					$parkir=mysqli_fetch_row($cek_parkir);
					$cek_parkiran=mysqli_num_rows($cek_parkir);
					if ($cek_parkiran > 0)
					{
					$now=(new \DateTime())->format('Y-m-d H:i:s');
	mysqli_query($con, "INSERT INTO tiket_parkir (id_tiket, id_parkir, jam_masuk, jenis_kendaraan) VALUES ('$id_baru', '$parkir[0]', '$now', 'Motor')");
	...
	echo "<script>alert('Input Parkiran Berhasil !');window.location = 'newtab.php?id_tiket=$id_baru'</script>";
		...
		echo "<script>alert('Maaf parkiran penuh :( silakan parkir di pinggir jalan :D');window.location = 'index.php?page=pengguna-motor'</script>";
	}
}
	...
	elseif ($hal=='tempat_parkir' AND $act=='input_tempat_parkir'){
		mysqli_query($con,"INSERT INTO tempat_parkir(id_parkir, blokir) VALUES('$_POST[id_parkir]','$_POST[blokir]')");
